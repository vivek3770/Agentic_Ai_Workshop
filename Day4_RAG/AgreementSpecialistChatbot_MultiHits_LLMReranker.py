...
class AgreementSpecialistChatbotMultiHitsLLMReranker:
    def __init__(self, llm_model: str = "gemini-2.5-flash") -> None:
        ...
        # Use OpenAI format for history to be compatible with QueryRewriter
        self.history: List[Dict[str, str]] = [{"role": "system", "content": SYSTEM_PROMPT}]
        self.rewriter = QueryRewriter(model=llm_model)
        self.reranker = LLMReranker(model="gemini-2.5-flash") # LLMReranker defaults to gemini-2.5-flash now too if init args changed

    def _context_block(self, hits: List[Dict]) -> str:
        if not hits:
            return ""
        lines: List[str] = ["Relevant context from FAQ (LLM re-ranked top-3):"]
    ...
    def ask(self, user_message: str) -> str:
        ...
        top_hits = self.reranker.rerank(rewritten_query, hits, top_r=3)
        print("top hits from LLM reranker", top_hits)
...
def _repl() -> None:
    bot = AgreementSpecialistChatbotMultiHitsLLMReranker()
    print("Agreement Specialist Chatbot (Multi-Hits + LLM Reranker). Type 'exit' to quit.\n")
    while True:
        try:
